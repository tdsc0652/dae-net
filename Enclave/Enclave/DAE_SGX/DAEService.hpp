...
public:
    ...
    ring_id_t get_dae_node_id(RelayConfig *config) {
        ...
        //uint32_t random_relay_id = sgx_randombytes_random() % config->max_relay_id;
    ...
    DAEService(RelayConfig* config)
    {
        ...
        //size_t len = strlen(config->relay_ip) + 1;
        //strncpy_s(node_ip, len, config->relay_ip, len);
        node_ip = config->relay_ip;
    ...
    void start(){
        ...
        bool check = false;
        ocall_log_info("daenode join");
        dae_node->join(joiner_node, check);

        //DLOG(INFO) << "Back to DAEService.start()" ;
        ocall_log_info("libevent start");
        ocall_sgx_libevent_start();
