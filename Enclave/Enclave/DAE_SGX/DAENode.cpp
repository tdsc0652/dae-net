...
std::function<void()> p2p_maintain_task;
std::function<void()> p2p_joined_successful_once;
std::function<void()> p2p_fetch_successor_list;
std::function<void()> p2p_check_successor_failure_once;
std::function<void()> p2p_check_predecessor_failure_once;
std::function<void()> p2p_verify_successor_failure_once;
std::function<void()> p2p_verify_predecessor_failure_once;


//ecall
void ecall_p2p_maintain_task(){
    p2p_maintain_task();
}
void ecall_p2p_joined_successful_once(){
    p2p_joined_successful_once();
}
void ecall_p2p_fetch_successor_list(){
    p2p_fetch_successor_list();
}
void ecall_p2p_check_successor_failure_once(){
    p2p_check_successor_failure_once();
}
void ecall_p2p_check_predecessor_failure_once(){
    p2p_check_predecessor_failure_once();
}
void ecall_p2p_verify_successor_failure_once(){
    p2p_verify_successor_failure_once();
}
void ecall_p2p_verify_predecessor_failure_once(){
    p2p_verify_predecessor_failure_once();
}

//ocall-wrapper
void sgx_addevent_p2p_maintain_task(struct timeval *interval, size_t n, int boottime){
	if(ocall_sgx_addevent_p2p_maintain_task(interval, n, boottime) != SGX_SUCCESS){
		log_warn(LOG_WARN, "error!!");
	}
}
void sgx_addevent_p2p_joined_successful_once(struct timeval *interval, size_t n){
	if(ocall_sgx_addevent_p2p_joined_successful_once(interval, n) != SGX_SUCCESS){
		log_warn(LOG_WARN, "error");
	}
}
void sgx_addevent_p2p_fetch_successor_list(struct timeval *interval, size_t n){
	if(ocall_sgx_addevent_p2p_fetch_successor_list(interval, n) != SGX_SUCCESS){
		log_warn(LOG_WARN, "error");
	}
}
void sgx_addevent_p2p_check_successor_failure_once(struct timeval *interval, size_t n){
    if(ocall_sgx_addevent_p2p_check_successor_failure_once(interval, n) != SGX_SUCCESS){
        log_warn(LOG_WARN, "error");
    }
}
void sgx_addevent_p2p_check_predecessor_failure_once(struct timeval *interval, size_t n){
    if(ocall_sgx_addevent_p2p_check_predecessor_failure_once(interval, n) != SGX_SUCCESS){
        log_warn(LOG_WARN, "error");
    }
}
void sgx_addevent_p2p_verify_successor_failure_once(struct timeval *interval, size_t n){
    if(ocall_sgx_addevent_p2p_verify_successor_failure_once(interval, n) != SGX_SUCCESS){
        log_warn(LOG_WARN, "error");
    }
}
void sgx_addevent_p2p_verify_predecessor_failure_once(struct timeval *interval, size_t n){
    if(ocall_sgx_addevent_p2p_verify_predecessor_failure_once(interval, n) != SGX_SUCCESS){
        log_warn(LOG_WARN, "error");
    }
}
...
void DAENode::join_with_help(const PeerPair &joiner_node) {
    bool is_find = false;
    //std::cout << "join\n";
    this->protocol->join_find_successor(joiner_node.ip, joiner_node.relay_id,
    ...
    sgx_addevent_p2p_joined_successful_once(&t, sizeof(timeval));
...
void DAENode::join(const PeerPair &joiner_node, bool &is_find){
    ...
    if(strcmp(this->Node_ip, joiner_node.ip) != 0 || this->port != joiner_node.port) {
    ...
    sgx_addevent_p2p_maintain_task(&time_internal, sizeof time_internal, bootstrap_time);
    ...
    if(config->check_failure){
        ...
        p2p_fetch_successor_list = [this](){
            fetch_successor_list();
        };
        sgx_addevent_p2p_fetch_successor_list(&tv, sizeof tv);
    }
}
...
void DAENode::check_predecessor_failure(){
    if(!this->predecessor_onchecking && this->predecessor!= -1){
        ...
        sgx_addevent_p2p_verify_predecessor_failure_once(&tv, sizeof tv);
    ...
    if(!this->successor_onchecking && this->finger_table[0].successor != this->id){
        ...
        p2p_verify_successor_failure_once = [](){
            if (!current_node->successor_status) {
                ...
                const char* successor_ip = current_node->successor_list.front().ip;
        ...
        sgx_addevent_p2p_verify_successor_failure_once(&tv, sizeof tv);
    }
}
