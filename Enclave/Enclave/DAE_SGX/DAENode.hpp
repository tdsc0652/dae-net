...
#include <string.h>
#include "orconfig.h"
#include "ConcurrentBlockQueue.hpp"
...
public:
    ...
    MsgDescriptor(bool is_dd, int session, int seq, uint64_t msg_type, ring_id_t connector_id, ring_id_t client_key, char *buf, int size){
        ...
        memcpy_s(info.msg, size, buf, size);
...
public:
    ...
    explicit DAENode(int _m, ring_id_t _id, int _port, const char* _addr, int bootstrap_t, RelayConfig *_config, DAEProtocol* _pro):m(_m), bootstrap_time(bootstrap_t){
        ...
        this->id = _id;
        size_t  addr_len = strlen(_addr) + 1;
        strncpy_s(Node_ip, addr_len,_addr, addr_len);
        this->port = _port;

        // predecessor initialize
        this->predecessor = -1;
        strncpy_s(predecessor_ip, addr_len, _addr, addr_len); 
        ...
        for (int i = 0;i < m;i++) {
            ...
            strncpy_s(finger_table[i].ip, addr_len, _addr, addr_len);
...
//    }
    ...
    void add_to_successor_list(ring_id_t id, PeerPair& peer){
        ...
        //strncpy_s(backup_successor.ip, )
        strncpy_s(backup_successor.ip, strlen(peer.ip) + 1, peer.ip, strlen(peer.ip) + 1);
        backup_successor.port = peer.port;
        backup_successor.relay_id = peer.relay_id; 
    ...
    void set_predecessor_of_successor_ip(const char* ip){
        ...
        strncpy_s(this->predecessor_of_successor_ip, ip_len, ip, ip_len);
    }
    ...
    void updatesuccessor_ip(int k, const char* _ip) {
        size_t ip_len = strlen(_ip) + 1;
        strncpy_s(this->finger_table[k].ip, ip_len, _ip, ip_len);
    }
    ...
    void setpredecessor_ip(const char* ip) {
        size_t ip_len = strlen(ip) + 1;
        strncpy_s(this->predecessor_ip, ip_len, ip, ip_len);
    }
