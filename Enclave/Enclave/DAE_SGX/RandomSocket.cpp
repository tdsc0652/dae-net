...
//}
std::function<void()> shuffle_start;
void ecall_shuffle_start(){
    shuffle_start();
}

void sgx_shuffle_start(struct timeval *tv, size_t len){
    if(ocall_sgx_shuffle_start(tv, len) != SGX_SUCCESS){
        abort();
    }
}

in_addr_t sgx_inet_addr(const char *arg){
    in_addr_t res;
    if(ocall_sgx_inet_addr(&res, arg) != SGX_SUCCESS){
        return -1;
    }
    return res;
}
int sgx_inet_aton(const char *arg, struct in_addr *_inp, size_t n){
    int res = -1;
    if(ocall_sgx_inet_aton(&res, arg, _inp, n) != SGX_SUCCESS){
        return -1;
    }
}
...
int sgx_bind(int fd, const struct sockaddr *addr , int n){
    int res;
    if(ocall_sgx_bind(&res, fd, addr, n) != SGX_SUCCESS){
        return -1;
    }
    return res;
}
...
void RandomSocket::DAE_direct_send(std::unique_ptr<AnonymousPacket> packet, const char* _ip_addr, int dest_port) {
    ...
    server_addr.sin_addr.s_addr = sgx_inet_addr(_ip_addr);

    int t = sgx_sendto(socket_fd, (void*)(packet.get()), packet->total_size(), 0, (const struct sockaddr *) &server_addr, sizeof(server_addr));
    if (t == -1) {
...
static void call_back_func1(void *){
        socklen_t sock_len;
        struct sockaddr_in client_addr;
	while(true) {
        std::unique_ptr<AnonymousPacket> r_packet(new AnonymousPacket()); //auto r_packet = std::make_unique<AnonymousPacket>();
        int size = sgx_recvfrom(current_socket->socket_fd, (void*)r_packet->buf, MAX_MTU, 0, ( struct sockaddr *) &client_addr, &sock_len);
...
bool RandomSocket::init_DAEService() {
    ...
    if ( (socket_fd = sgx_socket(AF_INET, SOCK_DGRAM, 0)) < 0 ) {
    ...
    if ( sgx_bind(socket_fd, (const struct sockaddr *)&server_addr,
    ...
    int _x = 0;
    auto pid = sgx_pthread_create_call_back_func1(call_back_func1, 0, &_x, sizeof(_x));
    //detach
    sgx_pthread_detach(pid);
    ...
    sgx_shuffle_start(&tv, sizeof(tv));
    ...
    switch (id) {
        ...
        case message::internal_reply_successor_list: {
            ...
            if(su_id != this->in_socket_node_instance->id){
                ...
                PeerPair backup_successor;
                strncpy(backup_successor.ip, su_ip, strlen(su_ip) + 1);
                backup_successor.port = su_port;
                backup_successor.relay_id = su_id;
        ...
        case message::external_pingpong_service: {
            ...
            if(is_dest){
                ring_id_t pre_client_key = packet->header()->relay_reply_id;
                uint64_t reply_msg_type = message::external_reply_pingpong_service;
                int session_id = packet->header()->session_id;
...
host_port_t RandomSocket::make_host(const std::string host, int port, uint64_t id) {
    ...
    sgx_inet_aton(host.c_str(), &t.sock.sin_addr, sizeof(in_addr));
    return t;
}
